		...
		<section className="space-y-6">
			...
			<div className="space-y-2">
				...
				<div className="flex gap-2 flex-wrap items-center">
					<input className="input" placeholder="Nueva congregación" value={newCong} onChange={(e) => setNewCong(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') addCong(); }} />
				...
				<div className="flex gap-2 flex-wrap">
					...
					{congs.length === 0 && (
						<span className="text-xs text-[color:var(--muted)]">Aún no hay congregaciones</span>
					)}
				</div>
			</div>
						...
						<div key={slot} className="card p-4">
							<div className="flex items-center justify-between gap-3 flex-wrap">
								...
								<div className="flex items-center gap-3 flex-wrap">
									...
									<select className="select disabled:opacity-60" disabled={!plantilla} value={asign?.congregacion || ''} onChange={async (e) => { const v = e.target.value; if (!v) return; await upsertSlot(slot, { congregacion: v }); }}>
										<option value="">Asignar congregación…</option>
										{(congs || []).map((c) => (<option key={c} value={c}>{c}</option>))}
									</select>

									<label className="inline-flex items-center gap-2 text-sm">
										<input type="checkbox" checked={!!asign?.completado} disabled={!puedeCompletar} title={!puedeCompletar ? 'Selecciona plantilla, congregación y tareas' : 'Marcar completado'} onChange={async (e) => { await upsertSlot(slot, { completado: e.target.checked }); }} />
										Completado
									</label>
								</div>
							</div>
										...
										{tpl.tareas.map((t) => {
											...
											return (
												<label key={t.area} className="flex items-start gap-2 rounded px-3 py-2 bg-[color:var(--surface-2)] border border-[color:var(--border)]">
													<input type="checkbox" className="mt-1" disabled={!asign?.congregacion} checked={checked} onChange={async (e) => { const next = new Set(tareasElegidas); if (e.target.checked) next.add(t.area); else next.delete(t.area); await upsertSlot(slot, { tareas: Array.from(next) }); }} />
													<div>
