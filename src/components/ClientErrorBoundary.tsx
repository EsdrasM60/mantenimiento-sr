...
export default class ClientErrorBoundary extends React.Component<Props, { error: Error | null; info: string | null }> {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo) {
    ...
    // Ensure componentStack is a string or null (avoid undefined)
    ...
    try {
      fetch('/api/debug/log', {
        ...
        body: JSON.stringify({ message: error.message, stack: info.componentStack ?? null, url: typeof window !== 'undefined' ? window.location.href : null }),
      }).catch(() => {});
    } catch (e) {
      // ignore
    }
